...
export function LanguageModal({ isOpen, onClose }: LanguageModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in">
      <div className="w-full max-w-sm bg-white rounded-3xl p-6 shadow-2xl border border-sky-100 relative">
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">{t("chooseLanguage")}</h3>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-3.5 rounded-2xl border text-left transition-all cursor-pointer ${
                  isSelected
                    ? "border-[#0284C7] bg-[#0284C7]/5 text-[#0284C7] font-bold"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-400 font-normal">{lang.native}</div>
                  </div>
                </div>
                {isSelected && <Check className="w-5 h-5 text-[#0284C7]" />}
              </button>
            );
          })}
        </div>

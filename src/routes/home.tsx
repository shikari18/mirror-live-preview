...
import { Settings, Bell, MapPin, Check, Volume2, Play, Sparkles, Plus, CloudRain, Zap, Camera, UserCheck, AlertCircle, Building2, ChevronRight, Activity } from "lucide-react";
...
export function HomePage() {
  ...
  return (
    <PhoneFrame>
      ...
      {!userLocation && (
        <div className="mx-5 mt-3 p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-900 flex items-center justify-between text-xs animate-in fade-in shadow-xs">
          <div className="flex items-center gap-2 font-bold">
            <AlertCircle className="w-4 h-4 text-amber-600 shrink-0" />
            <span>Almost done! Add your farm location to complete setup</span>
          </div>
          <Link to="/profile" className="px-3 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-extrabold shrink-0 shadow-xs cursor-pointer">
            Complete
          </Link>
        </div>
      )}
      ...
      {/* Header Bar */}
      <header className="px-5 pt-4 pb-3.5 flex items-center justify-between border-b border-gray-200/80 bg-white/80 backdrop-blur-md sticky top-0 z-30">
        <div className="flex items-center gap-3">
          <Link to="/settings" className="p-2 text-gray-700 hover:text-[#0F6236] hover:bg-gray-100/80 rounded-2xl transition-all cursor-pointer">
        ...
        <div className="flex items-center gap-3">
          <Link to="/notifications" className="relative p-2 rounded-2xl hover:bg-gray-100/80 transition-all cursor-pointer">
            <Bell className="w-5.5 h-5.5 text-gray-800" />
            <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-white animate-pulse" />
          </Link>
      ...
      {/* Billion-Dollar Hero Status Card */}
      <section className="mx-5 mt-4 rounded-3xl bg-gradient-to-br from-[#0B2B1B] via-[#0F6236] to-[#062817] text-white p-5 relative overflow-hidden shadow-2xl shadow-[#0F6236]/30 border border-emerald-500/20">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-[11px] font-extrabold uppercase tracking-wider text-emerald-300 bg-white/10 px-3 py-1 rounded-full backdrop-blur-md border border-white/10">
              <Sparkles className="w-3.5 h-3.5 animate-spin" /> {farmName} Live Status
            </div>
          ...
          <div className="mt-1 text-[13px] text-emerald-100 font-medium leading-relaxed">
            {pondsCount > 0 ? `Total Stock: ${totalFish.toLocaleString()} Fish • AI Doctor Synced` : "Measure your pond with live camera scanner to sync metrics"}
          </div>
      ...
      {/* Live Weather & Voice Advisory Card */}
      ...
      <section className="px-5 mt-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {quickActions.map(({ img, label, tint, to }) => {
            const inner = (
              <>
                <div className={`w-full aspect-square rounded-2xl border flex items-center justify-center ${tint} shadow-xs hover:scale-105 transition-all duration-300`}>
      ...
      <section className="mx-5 mt-5 mb-6 rounded-3xl bg-gradient-to-br from-[#0F6236]/15 via-[#0F6236]/10 to-emerald-50 p-5 relative overflow-hidden border border-[#0F6236]/25 shadow-md">
        <div className="max-w-[62%]">
          ...
          <div className="text-[11.5px] text-gray-600 mt-1 font-medium leading-relaxed">Order high quality feeds at wholesale market rates with free farm delivery.</div>
...
function StatCard({ tint, img, label, value, sub }: { tint: string; img: string; label: string; value: string; sub: string }) {
  return (
    <div className={`bg-white rounded-2xl p-3.5 flex items-center gap-3 border border-gray-200/90 shadow-xs hover:border-[#0F6236]/30 transition-all`}>
